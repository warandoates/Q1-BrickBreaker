...
function preload() {
    ...
    brickBreak.load.image("gameOverBtn", "img/gameOver.png");
    brickBreak.load.image('paused', 'img/paused.png');
...
function create() {
    ...
    paddle = brickBreak.add.sprite(regWidth, 550, "paddle");
...
function textMaker() {
    ...
    textScore = brickBreak.add.text(650, 20, `Score: ${score}`, textInfo);
    highScore = brickBreak.add.text(650, 65, `HighScore: ${highScoreVal}`, textInfo);
...
function restart(ball) {
  ...
  roundText.destroy();
  brickField.destroy();
  gameOverBtn = brickBreak.add.button(regWidth, brickBreak.world.height - 200, "gameOverBtn", create);
  gameOverBtn.scale.setTo(0.5);
  gameOverBtn.anchor.set(0.5);
}

function paddleReset() {
    paddle.reset(regWidth, 550);
    paddleMvmt = false;
}
